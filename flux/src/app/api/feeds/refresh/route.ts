...
export async function POST(request: Request) {
  ...
  const results: { feedId: string; title: string; newItems: number; error?: string }[] = [];
  ...
  for (const { feed } of userFeed) {
    ...
    if (result.success) {
      ...
      results.push({ feedId: feed.id, title: result.feedTitle, newItems });
    } else {
      results.push({ feedId: feed.id, title: feed.title ?? "Unknown", newItems: 0, error: result.error });
    }
  }
